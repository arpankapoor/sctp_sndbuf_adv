...
A dumbbell shaped network topology is created with 2 routers in the center, and
multiple hosts connected to each of the routers.

\begin{figure}[h]
  \centering
  \small
  \begin{tikzpicture}[start chain=testbed going right, node distance=1.5cm]
    ...
    \node
    [on chain, join=with pc1, join=with pcn,
    label=below:Router\textsubscript{\(1\)}]
    {\includegraphics{imgs/router.eps}};

    \node[on chain, join,
    label=below:Router\textsubscript{\(2\)}] (router2)
    {\includegraphics{imgs/router.eps}};

    \node [on chain] {\vdots};

    \node [start branch=3 going above, on chain, join=with router2,
    label=below:PC\textsubscript{\(x+1\)}]
    {\includegraphics{imgs/pc.eps}};

    \node [start branch=4 going below, on chain, join=with router2,
